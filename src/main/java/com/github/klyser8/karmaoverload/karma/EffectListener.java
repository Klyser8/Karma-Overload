...
public class EffectListener implements Listener {
    ...
    public void onMobDeath(EntityDeathEvent event) {
        ...
        Alignment alignment = plugin.getProfileProvider().getProfile(player).getAlignment();
    ...
    public void onKarmaGain(KarmaGainEvent event) {
        ...
        debugMessage(plugin, player.getName() + " gained " + event.getGainedKarma() + " karma from " + event.getSource(), DebugLevel.HIGH);
    ...
    public void onKarmaLoss(KarmaLossEvent event) {
        KarmaProfile profile = event.getProfile();
        Player player = profile.getPlayer();
        debugMessage(plugin, player.getName() + " lost " + event.getLostKarma() + " karma from " + event.getSource(), DebugLevel.HIGH);
        if (event.getSource() == KarmaSource.COMMAND || event.getSource() == KarmaSource.VOTING) return;
        Alignment alignment = profile.getAlignment();
        if (!alignment.getKarmaEffects().containsKey(KARMA_MULTIPLIER)) return;
        KarmaMultiplierEffect effect = (KarmaMultiplierEffect) alignment.getKarmaEffects().get(KarmaEffectType.KARMA_MULTIPLIER);
    ...
    public void onAlignmentChange(AlignmentChangeEvent event) {
        KarmaProfile profile = event.getProfile();
        Player player = profile.getPlayer();
        debugMessage(plugin, player.getName() + "'s alignment went from  " + event.getOldAlignment().getName() + " to " + event.getNewAlignment().getName(), DebugLevel.HIGH);
    ...
    /*private void createHelix(KarmaProfile profile, Alignment alignment) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                Location particleLoc = new Location(loc.getWorld(), loc.getX() + x, loc.getY() + y, loc.getZ() + z);
                if (y > 25) {
                    loc.getWorld().spawnParticle(Particle.FIREWORKS_SPARK, particleLoc, 50, 0, 0, 0, 0.25);
                    loc.getWorld().playSound(particleLoc, Sound.ENTITY_FIREWORK_ROCKET_LARGE_BLAST_FAR, 2.0f, 1.0f);
                    cancel();
                }
                loc.getWorld().spawnParticle(Particle.REDSTONE, particleLoc,
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                y+=0.5;
                radius-=0.02;
                double x = radius * Math.sin(y + Math.PI);
                double z = radius * Math.cos(y + Math.PI);
                Location particleLoc = new Location(loc.getWorld(), loc.getX() + x, loc.getY() + y, loc.getZ() + z);
                if (y > 25) {
                    loc.getWorld().spawnParticle(Particle.FIREWORKS_SPARK, particleLoc, 50, 0, 0, 0, 0.25);
                    loc.getWorld().playSound(particleLoc, Sound.ENTITY_FIREWORK_ROCKET_LARGE_BLAST_FAR, 2.0f, 1.0f);
