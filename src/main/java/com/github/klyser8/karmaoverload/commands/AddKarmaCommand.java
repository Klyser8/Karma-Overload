...
public class AddKarmaCommand extends CommandBase {
    ...
    public AddKarmaCommand(KarmaOverload plugin) {
        this.plugin = plugin;
    ...
    public void addCommand(CommandSender sender, Double amount, String targetName, @Optional String reason) {
        ...
        if (sender.getName().equalsIgnoreCase(targetName)) {
            sender.sendMessage(lang.getMessage(Message.SCORE_ADDED_SUCCESSFULLY_SELF).replace("<NUMBER>", String.valueOf(amount)));
        } else {
            sender.sendMessage(lang.getMessage(Message.SCORE_ADDED_SUCCESSFULLY).replace("<PLAYER>", targetName)
