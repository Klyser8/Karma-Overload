...
public class TableGenerator {
    ...
    private final static List<Character> char7 = Arrays.asList('°', '~', '@');
    private final static List<Character> char5 = Arrays.asList('"', '{', '}', '(', ')', '*', 'f', 'k', '<', '>');
    private final static List<Character> char4 = Arrays.asList('I', 't', ' ', '[', ']', '€');
    private final static List<Character> char3 = Arrays.asList('l', '`', '³', '\'');
    private final static List<Character> char2 = Arrays.asList(',', '.', '!', 'i', '´', ':', ';', '|');
    private final static char char1 = '\u17f2';
    private final static Pattern regex = Pattern.compile(char1+"(?:§r)?(\\s*)"
			+ "(?:§r§8)?"+char1+"(?:§r)?(\\s*)"
			+ "(?:§r§8)?"+char1+"(?:§r)?(\\s*)"
			+ "(?:§r§8)?"+char1);
    private final  static String colors = "[&§][0-9a-fA-Fk-oK-OrR]";
    private final  TableAlignment[] alignments;
    private final  List<Row> table = new ArrayList<>();
    private final  int columns;
   
    public TableGenerator(TableAlignment... alignments) {
    ...
    public List<String> generate(Receiver receiver, boolean ignoreColors,
        ...
        for (Row r : table) {
            ...
            for (int i = 0; i < columns; i++) {
                TableAlignment agn = alignments[i];
                String text = r.texts.get(i);
                int length;
                ...
                if (agn == TableAlignment.LEFT) {
                    sb.append(text);
                    if (i < columns - 1)
                    	sb.append(char1s).append(spaces);
                }
                if (agn == TableAlignment.RIGHT) {
                    sb.append(spaces).append(char1s).append(text);
                }
                if (agn == TableAlignment.CENTER) {
